#!/usr/bin/env python3
"""dbs 模板生成器：生成 DBS 毛玻璃风幻灯片（tomabc-deck 技能 · templates/dbs）。
...
设计系统来源：tomabc-html / dbs-html 的 DBS Glass（粉紫渐变 + 彩色光斑 + 半透明白玻璃卡 + DBS 红）。
样式文件 templates/dbs/dbs-deck.css 是该系统的 1920×1080 定尺改造版；组件 class 与 news 模板同名，
两套模板的内容可互相平移（只换 CSS 与壳，slides 基本不动）。
...
脚本行为：
  1. 以 SHELL 为底：替换头部注释 / <title> / <style>（→ dbs-deck.css）/ page-label / 顶栏 logo
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  风格：dbs 模板（DBS 毛玻璃风 · 粉紫渐变底 + 彩色光斑 + 半透明白玻璃卡 + DBS 红点缀 · 系统黑体 Noto Sans CJK SC）
  结构：tomabc-deck 16 步标准结构 + 6 Phase 管线（gen_deck → narrations → TTS → 录屏 → 发布）
  顶栏：完整 DBS logo（内联 base64，规范见 dbs-html 技能）
  用法：
...
# ═════════════════════════════ ② 壳与输出 ═════════════════════════════
SKILL_TEMPLATE = "/home/nvidia/.claude/skills/tomabc-deck/templates/dbs"
...
def main():
    ...
    # 3. <style> 块整体替换为 dbs-deck.css（先过注释陷阱检查，注释坏了后面全白搭）
    css_path = os.path.join(SKILL_TEMPLATE, "dbs-deck.css")
    ...
    # 4b. 顶栏 logo → DBS 完整 logo（内联 base64 PNG）
